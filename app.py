...
def filter_data_detail(reg,vaccin):
		...
		return df_vaccin_detail.query('reg==@reg & vaccin==@vaccin').to_json()
...
# CHargement liste des rÃ©gions
src = 'C:/Users/Utilisateur/PycharmProjects/montee_en_competence/stat_pop.csv'

# df_population=pd.read_csv(src,sep=";")///////////////a remettre
# bilan des donnÃ©es actuelle
# Le fichier est mis Ã  jour quotidiennement et il comprend la somme par rÃ©gion et par vaccin. Cependant pour certaines
# rÃ©gions et vaccins,la valeur peut manquer Ã  un jour donnÃ©. On crÃ©ee donc un fichier Ã  jour cumulant les donnÃ©es
# de la journÃ©e selon data-france et celles donnÃ©es par data-fr&ance il y a pluysieurs jours pour les valeurs manquantes
...
def maj_data_complete():
	...
	if path.exists(csv_today_path + "r"):
		...
		# recherche de donnÃ©es dans des anciennes stats

		day_diff = 0
		# for reg in reg_sans_data:
		data_full = False
		# boucle sur les archives de donnÃ©es
		while not data_full:
			day_diff += 1
			today = dt.today().strftime("%m-%d-%Y")
		...
		if not data_full:
			print("data pas complÃ¨te")
			exit()

		df_reg_manquant.sort_values(['jour'], inplace=True)
		# on reprend la derniÃ¨re ligne du fichier de donnÃ©es manquante
...
# somme des datas par jour mÃªme incomplÃ¨te pour pouvoir viusaliser les changements depuis lavaeille
#dans df_vaccin_detail on peut avoir des journÃ©es manquantes
@app.route('/b')
def make_vaccin_detail():
	...
	#le seul truc qui nous manque c'est le cumul par jour par vaccin sur toutes les rÃ©gions

df_vaccin_detail=make_vaccin_detail()
# month_options = [{'label': month, 'value': i}
#                  for (month,i) in enumerate(pd.unique(df_liste_vaccin))]

#somme_jour_vaccin tient compte du fait qu'il peut y  voir plusieurs ligns pour une meme journÃ©e et uneme meme re
...
def ratio():
	...
	pop_totale_vaccinÃ© = population_totale / df_vaccin_quotidien["n_dose1"].sum()
